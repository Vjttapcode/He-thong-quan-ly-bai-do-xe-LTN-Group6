...
// WiFi + MQTT config
#define WIFI_SSID       "COM DUNG LE" //chỉ kết nối 4G
#define WIFI_PASSWORD   "khongbiet"
#define TOKEN           "Yu3K96In0p9IkfvODYKo"
...
void loop() {
  ...
  Serial.print("Dist In: "); Serial.print(dist_in);
  Serial.print(" cm, Dist Out: "); Serial.print(dist_out); Serial.println(" cm");

  if(detectbefor){
    if(dist_in >= 11) {
      detectbefor = false;
      delay(1000);
      dist_in = readDistanceCM(TRIG_IN, ECHO_IN);
    }
  }
  ...
  if (dist_in > 0 && dist_in <= 4 && !detectbefor) {
    if (Gateleft) {
    ...
    detectbefor = true;
  } else if (dist_in >= 5 && dist_in <= 10 && !Gateleft && !detectbefor) {
    if (!Gateright) {
      Serial2.print("DETECT");
      Gateright = true;
      while (!detectcar && !detectmotor) {
